...
def callback(locals_, globals_):
    ...
    if env.done:
        ...
        summary2 = tf.Summary(value=[tf.Summary.Value(tag='% improvement wrt mff', simple_value=over_mff)])
...
if args.retrain == "no_retrain":
	...
	model = DQN(policy, env, learning_rate=args.lr, prioritized_replay=True, verbose=1,tensorboard_log=Path(os.getenv("HOME")+"/tensorboard/"+args.model_name))
	# Train the agent
	...
	path = Path(os.getenv("HOME")+"/models/"+args.model_name)
	model = DQN.load(path,tensorboard_log=Path(os.getenv("HOME")+"/tensorboard/"+args.model_name+"_retrained"))
	model.set_env(env)
	model.learn(total_timesteps=int(args.steps),callback=callback)
	# Save the agent
	path = Path(os.getenv("HOME")+"/models/"+args.model_name+"_retrained")
	model.save(path)
